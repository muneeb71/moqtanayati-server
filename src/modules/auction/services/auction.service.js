...
class AuctionService {
  async createAuction(data) {
    const requiredFields = [
      'productId', 'sellerId', 'title', 'description', 'startTime', 'endTime', 'startingBid', 'status'
    ];
  ...
  async getAuctionHistory() {
    return prisma.auction.findMany({ where: { OR: [{ status: 'ENDED' }, { status: 'CANCELLED' }] } });
  }
  ...
  async updateAuctionStatus(id, status) {
    const auction = await prisma.auction.update({ where: { id }, data: { status } });
    return auction;
  }
}
